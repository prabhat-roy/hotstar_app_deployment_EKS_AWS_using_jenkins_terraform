# Hotstar Clone (Next.js) → AWS EKS via Jenkins + Terraform

End-to-end CI/CD pipeline that builds hotstar-clone (Next.js / React), runs the full DevSecOps gate (SAST + SCA + image scanning), pushes to AWS ECR, and deploys to AWS EKS via Helm. The EKS cluster + supporting infra is provisioned by Terraform in the same repo.
...
```
├── src/                     ← hotstar-clone application source
├── hotstar-helm-chart/          ← Helm chart for the app
...
3. ECR repository named `hotstar-image` in account `873330726955` / region `us-east-1` (or update env block in `Jenkinsfile`).
...
```bash
helm uninstall hotstar-clone -n <namespace>
